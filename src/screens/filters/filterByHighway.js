...
const FilterByHighwayScreen = props => {
  ...
  const [selectedHighways, setSelectedHighways] =
    useState(filteredHighwaysData);
  ...
  const onReset = () => {
    ...
    navigation.navigate(routes.SCREEN_LISTING, {
      notification: t('filters.notifications.reset'),
    });
  };
  ...
  const onSubmit = () => {
    ...
    dispatchFilterListing();
    navigation.navigate(routes.SCREEN_LISTING, {
      notification: t('filters.notifications.applied'),
    });
  };

  return (
    <ScreenFilterWrapper
      backgroundImage={bgHighway}
      ...
      {highways.map((item, i) => {
        ...
        return (
          <ListTileCheckbox
            ...
            checked={checked}
            trailingIcon={
              <ImageBackground source={bgBadgeHighway} style={styles.wrapper}>
