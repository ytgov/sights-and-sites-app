    ...
    <Accordion
      ...
      renderHeader={(item, expanded) => (
        <View
          ...
          <Image
            source={expanded ? accordionOpenedIcon : accordionClosedIcon}
            resizeMode="contain"
            style={{width: 20, height: 20, marginRight: 17}}
          />
          <H3 style={{width: '90%'}} black>
            {item.title}
          </H3>
        </View>
      )}
      renderContent={item => (
        <View
          ...
          {item.url ? (
            <WebLink
              label={item.url_text ? item.url_text : 'More safe travel tips?'}
              url={item.url}
          ...
          {item.contacts
            ? item.contacts.map((m, i) => (
                <Body key={i} fontBold black style={{fontWeight: "bold"}}>
                  {m.name}
                  {m.contact}
                </Body>
              ))
